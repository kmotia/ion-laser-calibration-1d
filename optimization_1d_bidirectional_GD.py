...
def locate_best_mirror_pos(start=0, stop=1, step_size=0.1, precision=0.001, amp_min=80, min_step_size=0.0001):
    ...
    pos = mean                  # Set initial search position to mean of current fit
    step_size = precision
    while True:
        move_mirror_to_position(pos)
        ion_response = measure_ion_response(pos)
        ...
        if previous_response is not None:
            gradient = ion_response - previous_response
            
            # Adjust the learning rate based on the gradient
            if abs(gradient) > 1:      # High fluctuation
                step_size /= 2
            elif abs(gradient) < 0.1:  # Low fluctuation
                step_size *= 1.5
            else:                      # Normal fluctuation
                step_size *= 1.1

            # Ensure step size is within valid range
            step_size = max(min_step_size, min(step_size, (stop - start) / 3))

            # Update mirror position based on the gradient
            current_pos += np.sign(gradient) * step_size    # ability to move mirror in iether direction according depending if gradient is (+) or (-)
            current_pos = np.clip(current_pos, 0, 1)        # restrict mirror movement to [0, 1]. Don't want to break lab equipment!

        previous_response = ion_response


        # Grab all ion response data from ion_responses dictionary
        x_data = np.array(list(ion_responses.keys()))
        y_data = np.array(list(ion_responses.values()))

        # Try to find a gaussian fit with the desired amplitude and step_size
        popt = try_gaussian_fit(np.array(x_data), np.array(y_data))
        if popt is not None:
            amplitude, mean, stddev = popt
